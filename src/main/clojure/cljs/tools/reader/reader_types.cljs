...
(ns ^{:doc "Protocols and default Reader types implementation"
  ...
  (:require-macros [cljs.tools.reader.reader-types-macros :refer [update!]])
...
(deftype LineNumberingPushbackReader []
  IndexingReader
  (get-line-number [rdr]
    (throw (js/Error. "LineNumberingPushbackReader not yet implemented in ClojureScript")))
  (get-column-number [rdr]
    (throw (js/Error. "LineNumberingPushbackReader not yet implemented in ClojureScript")))
  (get-file-name [rdr] nil))
...
logging frame."
  ...
  (let [current-frame @source-log-frames]
    (subs (.toString ^StringBuffer (:buffer current-frame)) (:offset current-frame))))
...
(defn source-logging-push-back-reader
  ...
  ([s-or-rdr buf-len file-name]
     (SourceLoggingPushbackReader.
      ...
      0
      file-name
      (atom {:buffer (StringBuffer.)
             :offset 0}))))

(defn read-line
  "Reads a line from the reader or from *in* if no reader is specified"
  ([& args]
    (throw (js/Error. "read-line not supported in ClojureScript"))))
